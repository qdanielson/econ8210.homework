function [res] = residual_chebyshev_test(k,z,coefficients,chebyshev_function,z_grid, P, params)
    % Unpack params
    delta = params.delta;
    beta = params.beta;
    n_z = length(z_grid);
    ...
    % pull l value from approximation  
    l = max(funeval(coefficients, chebyshev_function, [k,z]),0);
    ...
    l1 = zeros(1, n_z);
    for zp = 1:n_z
        l1(zp) = max(funeval(coefficients, chebyshev_function, [k1,z_grid(zp)]),0);
    end
    ...
    res = (U_c(c) - beta * (1 - delta + r) * marginal_value)^2;
end